# -*- coding: utf-8 -*-
"""
Created on Thu Jul 18 10:56:49 2019
...
class stockGetter(object):
    ...
    def get_stock_name(self):
        ...
        df = pd.DataFrame(r)
        df["stock_id"] =df["Code"]+"."+df["ShowMkt"]
        df["stock_code"]=df["Code"]
        df["stock_name"]=df["Name"].map(lambda x: re.sub("\s*", '', x))
        df_stock_name= df[df['ShowMkt'].isin(['SZ','SH'])][["stock_name","stock_code","stock_id"]].reset_index(drop=True)
...
class yuqing_calculate_getter(object):
   ...
   def get_yuqing_score(self,date1,date2):
        ...
        # 计算个股的不同维度的舆情分数
            ...
            l_yuqing_score.append(df_temp_1)      
        
        df_yuqing_score=pd.concat(l_yuqing_score)
   ...
   def get_change_caculation(self,df1,df2,typ):
       ...
       for stock in df_stock_name["stock_code"].tolist():
            ...
            l_yuqing_change.append(df_temp)
        
       df_yuqing_change=pd.concat(l_yuqing_change)     
...
class stock_label(object):
    ...
    def stock_label_generate(self,typ): 
        ...
        change=int(df_label["{}_change".format(typ)].values[0])
        change_ratio=df_label["{}_change_ratio".format(typ)].values[0]
        ...
        def detail_text(typ,df_label):
            ...
            if change>0 and change_ratio!="/":                
                text="{}上升{}{},上升幅度{:.2%}".format(name_dic[typ],change,unit_dic[typ],change_ratio)
        
            if change<0 and change_ratio!="/":
                text="{}下降{}{},下降幅度{:.2%}".format(name_dic[typ],abs(change),unit_dic[typ],abs(change_ratio))

            if change>0 and change_ratio=="/":
                text="{}上升{}{}".format(name_dic[typ],change,unit_dic[typ])
            
            if change<0 and change_ratio=="/":
                text="{}下降{}{}".format(name_dic[typ],abs(change),unit_dic[typ])
            return text
        ...
        dic_label["detail"]=[{"change":change,"change_ratio":change_ratio}]
...
def main(): 
    ...
    for typ in list_tpye:
        ...
        dic_label_each={}
        dic_label_each["tg_self_yuqing_{}_positive".format(typ)]={}
        dic_label_each["tg_self_yuqing_{}_negative".format(typ)]={}
        #根据全部股票列表循环
        for count,stock in enumerate(list(dic_stock_name.keys())):
            if count % 100== 0:
            ...
            if stock_temp.sign=="positive" or stock_temp.sign=="negative":
                ...
                stock_id=df_each["stock_id"].values[0]
                label=df_each[column_name].values[0]
                dic[stock_id]=label
                dic_label_each[column_name].update(dic)
                
        dic_label_store[typ]=dic_label_each
